...
### Changed
- `/reopen` now attempts full side-effect rollback:
  - remove previously granted acceptance roles,
  - delete tracked acceptance announcement message,
  - send reopen-compensation DM to applicant when resolvable.
- Reopen command/thread summaries now include role, announcement, and DM rollback results.

### Fixed
- Acceptance role-assignment resolution now retries applicant lookup from submitted form fields (including case-insensitive username matching) when `applicantUserId` was missing.
- Denied/reopen DM flows now use the same fallback applicant resolution path.

## [1.4.2] - 2026-02-17

### Changed
- `/reopen` now attempts to revert roles previously granted during acceptance when reopening an accepted application.
- Reopen notices/logging now include role-revert result details.

### Fixed
- Updated reopen wording to clarify that DM/announcement side effects are still not automatically reverted.
...
### Changed
- Added `/msg` as an alias of `/message` with the same subcommands (`structured`, `embed`, `edit`).
- Removed legacy slash commands `/structuredmsg`, `/embedmsg`, and `/embededit` in favor of `/message` and `/msg`.
...
### Added
- New `/rr` reaction-role shortcut command using mode selection (`create`, `remove`, `list`, `gui`).
- New `/message` command using mode selection (`structured`, `embed`, `edit`).

### Changed
- Interaction routing now handles `/rr` and `/message` with the same mode-driven pattern used by `/set` and `/settings`.
- Legacy commands remain available: `/reactionrole`, `/structuredmsg`, `/embedmsg`, `/embededit`.
